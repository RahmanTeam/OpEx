...
def readConfigFile(scriptdir,configpath):
	ret=OrderedDict()
	if configpath==None: fn=scriptdir+'/config.txt'
	else: fn=configpath
	for line in open(fn):
		line=line.strip()
		if line=='': continue
...
parser.add_option('-i',"--input", default=None, dest='fastq', action='store', help="FASTQ file files")
...
params['MORECV']=''
if not options.full: params['MORECV']=params['MORECV']+'-c '+scriptdir+'/CoverView_default.json'
else: params['MORECV']=params['MORECV']+'-c '+scriptdir+'/CoverView_full.json'
